...
if [ -f "$(which lightningcss)" ]; then
	...
	if [ "$IS_BUILD" == "" ]; then
		esbuild --log-level=info --log-limit=0 --charset=utf8 --bundle --external:'*.svg' ${2:---minify --sourcemap} $3 "css/${1}/index.css" --outfile="dist/${1}.css" --watch
	else
		esbuild --log-level=warning --log-limit=0 --charset=utf8 --bundle --external:'*.svg' ${2:---minify --sourcemap} $3 "css/${1}/index.css" --outfile="dist/${1}.css"
	fi
else
